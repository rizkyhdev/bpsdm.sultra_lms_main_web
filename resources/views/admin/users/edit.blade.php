    ...
    <x-admin.card>
        <form action="{{ route('admin.users.update', $user) }}" method="POST" novalidate>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                ...
                <x-admin.input name="name" :label="__('Name')" :value="old('name', $user->name)" />
                <x-admin.input type="email" name="email" :label="__('Email')" :value="old('email', $user->email)" />
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 items-start">
            ...
            <div class="mt-6 flex items-center justify-between">
                <a href="{{ route('admin.users.index') }}" class="inline-flex items-center gap-2 px-3 py-2 rounded-md border border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                    <i class="fas fa-arrow-left"></i>
                    {{ __('Back') }}
                </a>
                <button type="submit" class="inline-flex items-center gap-2 px-3 py-2 rounded-md bg-indigo-600 text-white hover:bg-indigo-500">
