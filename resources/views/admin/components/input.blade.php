@props([
    'id' => null,
    'name' => null,
    'label' => null,
    'type' => 'text',
    'value' => null,
    'required' => false,
    'help' => null,
])

@php($inputId = $id ?? $name ?? uniqid('input_'))

<div {{ $attributes->merge(['class' => 'space-y-1']) }}>
    @if($label)
        <label for="{{ $inputId }}" class="block text-sm font-medium">{{ $label }} @if($required)<span class="text-red-600">*</span>@endif</label>
    @endif
    <input id="{{ $inputId }}" name="{{ $name }}" type="{{ $type }}" @class([
        'block w-full rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm px-3 py-2',
        'focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500',
        'border-red-300 text-red-900 placeholder-red-300 focus:ring-red-500 focus:border-red-500' => $errors->has($name),
    ]) value="{{ old($name, $value) }}" @if($required) required @endif />
    @if($help)
        <p class="text-xs text-gray-500">{{ $help }}</p>
    @endif
    @error($name)
        <p class="text-xs text-red-600">{{ $message }}</p>
    @enderror
</div>


