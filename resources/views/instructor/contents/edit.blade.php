...
function updateReview() {
  ...
  if (tipe === 'text' || tipe === 'html') {
    ...
    let fileText = '';
    if (fileInput.files && fileInput.files.length > 0) {
      fileText = `<p><strong>New File:</strong> ${fileInput.files[0].name}</p>`;
    } else if (hasExistingFile) {
      fileText = `<p><strong>Current File:</strong> {{ basename($content->file_path) }} (will be kept if no new file uploaded)</p>`;
    }
