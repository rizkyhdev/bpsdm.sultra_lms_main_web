...
<div class="container-fluid">
  <div class="card">
    <div class="card-header bg-primary text-white">
      <h5 class="mb-0">Module Edit Wizard</h5>
      <small>Edit module lengkap dengan sub-modules dan contents dalam satu alur</small>
    </div>
    <div class="card-body">
      @if(session('error'))
      ...
      <form action="{{ route('instructor.modules.update', $module->id) }}" method="post" enctype="multipart/form-data" id="moduleWizardForm">
        @csrf
        @method('PUT')
        ...
        <div class="wizard-step" id="step1">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="judul" value="{{ old('judul', $module->judul) }}" class="form-control" required>
            @error('judul')<small class="text-danger d-block">{{ $message }}</small>@enderror
          </div>

          <div class="mb-3">
            <label class="form-label">Deskripsi</label>
            <textarea name="deskripsi" class="form-control" rows="4">{{ old('deskripsi', $module->deskripsi) }}</textarea>
            @error('deskripsi')<small class="text-danger d-block">{{ $message }}</small>@enderror
          </div>

          <div class="mb-3">
            <label class="form-label">Urutan <span class="text-danger">*</span></label>
            <input type="number" name="urutan" value="{{ old('urutan', $module->urutan) }}" class="form-control" min="1" required>
            @error('urutan')<small class="text-danger d-block">{{ $message }}</small>@enderror
          </div>

          <div class="d-flex justify-content-end mt-4">
            <button type="button" class="btn btn-primary" onclick="nextStep(2)">Next: Manage Sub-Modules →</button>
          </div>
        </div>
        ...
        <div class="wizard-step" id="step2" style="display: none;">
          ...
          <p class="text-muted mb-4">Kelola sub-modules untuk module ini.</p>
          
          <div id="subModulesContainer">
            <!-- Existing and new sub-modules will be added here dynamically -->
          </div>

          <button type="button" class="btn btn-success mb-4" onclick="addSubModule()">
            <i class="bi bi-plus-circle"></i> Tambah Sub-Module Baru
          </button>
...
<template id="subModuleTemplate">
  <div class="card mb-3 sub-module-item" data-sub-module-index="" data-sub-module-id="">
    ...
    <div class="card-body">
      <input type="hidden" name="sub_modules[][id]" class="sub-module-id" value="">
      <div class="mb-3">
...
<template id="contentTemplate">
  <div class="card mb-3 content-item" data-content-index="" data-content-id="">
    ...
    <div class="card-body">
      <input type="hidden" name="sub_modules[][contents][][id]" class="content-id" value="">
      <div class="mb-3">
...
<script>
let subModuleIndex = {{ $module->subModules->count() }};
let contentIndex = {};
const existingSubModules = @json($module->subModules);
...
function validateStep2() {
  // Sub-modules are optional, so validation passes
  return true;
}

function loadExistingSubModules() {
  existingSubModules.forEach((subModule, idx) => {
    addSubModuleFromData(subModule, idx);
  });
}

function addSubModuleFromData(subModuleData, index) {
  const template = document.getElementById('subModuleTemplate');
  const clone = template.content.cloneNode(true);
  const subModuleItem = clone.querySelector('.sub-module-item');
  subModuleItem.setAttribute('data-sub-module-index', index);
  subModuleItem.setAttribute('data-sub-module-id', subModuleData.id);
  subModuleItem.querySelector('.sub-module-number').textContent = index + 1;
  ...
  idInput.name = `sub_modules[${index}][id]`;
  idInput.value = subModuleData.id;
  judulInput.name = `sub_modules[${index}][judul]`;
  judulInput.value = subModuleData.judul;
  deskripsiInput.name = `sub_modules[${index}][deskripsi]`;
  deskripsiInput.value = subModuleData.deskripsi || '';
  urutanInput.name = `sub_modules[${index}][urutan]`;
  urutanInput.value = subModuleData.urutan;
  
  // Load existing contents
  if (subModuleData.contents && subModuleData.contents.length > 0) {
    if (!contentIndex[index]) {
      contentIndex[index] = 0;
    }
    subModuleData.contents.forEach((content, contentIdx) => {
      addContentFromData(content, index, contentIdx);
      contentIndex[index]++;
    });
  } else {
    contentIndex[index] = 0;
...
function addSubModule() {
  ...
  idInput.name = `sub_modules[${subModuleIndex}][id]`;
  idInput.value = '';
...
function addContentFromData(contentData, subModuleIdx, contentIdx) {
  const subModuleItem = document.querySelector(`[data-sub-module-index="${subModuleIdx}"]`);
  ...
  idInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][id]`;
  idInput.value = contentData.id;
  judulInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][judul]`;
  judulInput.value = contentData.judul;
  tipeInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][tipe]`;
  tipeInput.value = contentData.tipe;
  urutanInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][urutan]`;
  urutanInput.value = contentData.urutan;
  
  if (htmlContentInput) {
    htmlContentInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][html_content]`;
    htmlContentInput.value = contentData.html_content || '';
  }
  if (fileInput) {
    fileInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][file_path]`;
  ...
  if (urlInput) {
    urlInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][external_url]`;
    urlInput.value = contentData.external_url || '';
  }
  if (youtubeUrlInput) {
    youtubeUrlInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][youtube_url]`;
    youtubeUrlInput.value = contentData.youtube_url || '';
  }
  if (requiredDurationInput) {
    requiredDurationInput.name = `sub_modules[${subModuleIdx}][contents][${contentIdx}][required_duration]`;
...
function renumberSubModules() {
  ...
  subModules.forEach((subModule, index) => {
    ...
    const idInput = subModule.querySelector('.sub-module-id');
    ...
    const subIdx = index;
    if (idInput) {
      idInput.name = `sub_modules[${subIdx}][id]`;
    }
...
function addContent(btn) {
  ...
  if (idInput) {
    idInput.name = `sub_modules[${subIdx}][contents][${contentIdx}][id]`;
    idInput.value = '';
  }
  ...
  if (requiredDurationInput) {
    requiredDurationInput.name = `sub_modules[${subIdx}][contents][${contentIdx}][required_duration]`;
  }
...
function removeContent(btn) {
  ...
  const subModuleItem = contentItem.closest('.sub-module-item');
  const subIdx = parseInt(subModuleItem.getAttribute('data-sub-module-index'));
  contentItem.remove();
  renumberContents(subModuleItem, subIdx);
}

function renumberContents(subModuleItem, subIdx) {
  ...
  contents.forEach((content, index) => {
    ...
    updateContentNames(content, subIdx, index);
  });
  contentIndex[subIdx] = contents.length;
}
...
function updateReview() {
  ...
  if (subModules.length === 0) {
    ...
    subModules.forEach((subModule, idx) => {
      ...
      const contents = subModule.querySelectorAll('.content-item');
      const isExisting = subModule.getAttribute('data-sub-module-id') !== '';
      const status = isExisting ? '(Existing)' : '(New)';
      const subModuleDiv = document.createElement('div');
      subModuleDiv.className = 'mb-2';
      subModuleDiv.innerHTML = `<strong>Sub-Module ${idx + 1}:</strong> ${judul} (${contents.length} contents) ${status}`;
