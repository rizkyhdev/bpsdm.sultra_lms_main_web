...
class CourseScheduleController extends Controller
{
    ...
    public function update(UpdateCourseScheduleRequest $request, $id): JsonResponse
    {
        ...
        // Convert local datetime input to UTC Carbon instances
        $course->start_date_time = !empty($validated['start_date_time'])
            ? \Carbon\Carbon::parse($validated['start_date_time'], config('app.timezone'))->setTimezone('UTC')
            : null;

        $course->end_date_time = !empty($validated['end_date_time'])
            ? \Carbon\Carbon::parse($validated['end_date_time'], config('app.timezone'))->setTimezone('UTC')
            : null;
